...
function VerificationForm() {
  ...
  useEffect(() => {
    ...
    if (flowId) {
      ory
        ...
        .catch(() => {
          setErrors(['Impossible de charger le flow de vérification.'])
          setIsLoading(false)
        })
      return
    }
    ...
    if (!code) {
      ory
        ...
        .catch(() => {
          setErrors(['Impossible d\'initialiser la vérification.'])
          setIsLoading(false)
        })
    } else {
      setIsLoading(false)
    ...
    if (code && flow && csrfToken && !verifying) {
      ...
      .catch((err: any) => {
        ...
        if (errorMessages.length === 0) {
          errorMessages.push(
            err?.response?.data?.error?.message || 'Erreur lors de la vérification.'
          )
        }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Success - redirect to geometrics
      window.location.href = 'https://geometrics.combaldieu.fr'
    } catch (err: any) {
      const data = err?.response?.data
      if (data?.ui) setFlow(data)
      ...
      if (errorMessages.length === 0) {
        errorMessages.push(
          err?.response?.data?.error?.message || 'Vérification impossible.'
        )
      }
        ...
        >
          <span className="flex items-center justify-center">
            {isSubmitting ? (
              ...
              <>
                {node.meta?.label?.text || 'Envoyer le lien de vérification'}
      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl border border-teal-200">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-3xl font-bold bg-gradient-to-r from-teal-600 to-yellow-500 bg-clip-text text-transparent">
            {verifying && errors.length === 0 ? 'Vérification en cours...' : errors.length > 0 ? 'Erreur' : 'Vérification'}
          </h1>
          <p className="mt-2 text-teal-700 opacity-80 text-center">
            {verifying && errors.length === 0 ? 'Veuillez patienter' : errors.length > 0 ? 'Une erreur est survenue' : 'Vérification de votre email'}
          </p>
        </div>
